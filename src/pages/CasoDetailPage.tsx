...
export default function CasoDetailPage() {
  ...
  }, [id, casos, navigate, seleccionarCaso]);
  ...
  const handleIniciarFlujoForense = () => {
    navigate(`/control/seguimiento-compliance?casoId=${caso.id}`);
  };
  ...
  return (
    <div className="space-y-6 apple-fade-in pb-12">
      ...
      {/* ── Back Link ── */}
      <Link to="/casos" className="flex items-center gap-2 text-xs font-bold text-[var(--co-accent)] hover:underline uppercase tracking-[0.2em] w-fit">
        <ArrowLeft size={14} />
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 apple-fade-in">
            {[
              ...
              <Card key={idx} className="flex flex-col justify-between p-5" hoverable={true}>
                ...
                <Link 
                  to={`${planilla.path}?casoId=${caso.id}`} 
