...
const ActaDictamenPage = () => {
  ...
  const handleCompilePdf = async () => {
    ...
    try {
      const blob = await generatePdfBlobFromElement(el, `Dictamen_Pericial_${caso?.numeroCaso || 'EXP'}`);
      setPdfBlob(blob);
      setTabIndex(1);
    } catch (err) {
  ...
  const handleDownloadZip = () => {
    downloadPlanillaZip(
      `Dictamen_Pericial_${caso?.numeroCaso || 'EXP'}`,
      'Dictamen Pericial Informático Forense — SHA256.US'
    );
  };
  ...
  return (
    <div className="planilla-container">
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <span style={{ color: '#FECF06', fontWeight: 700, fontSize: '13px' }}>
              Dictamen Pericial Informático Forense (ISO 27042 / COPP)
            </span>
          </Box>
      ...
      {tabIndex === 0 ? (
        ...
        <PlanillaPdfViewer
          pdfBlob={pdfBlob}
          title={`Dictamen Pericial — Caso #${caso?.numeroCaso || 'N/A'}`}
      ...
      <PlanillaGatingDialog
        ...
        nombrePlanilla="Dictamen Pericial Informático Forense"
      />
    </div>
  );
};
