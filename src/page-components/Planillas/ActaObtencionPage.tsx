...
const ActaObtencionPage = () => {
  ...
  const handleCompilePdf = async () => {
    ...
    try {
      const blob = await generatePdfBlobFromElement(el, `Acta_Obtencion_${caso?.numeroCaso || 'EXP'}`);
      setPdfBlob(blob);
      setTabIndex(1);
    } catch (err) {
  ...
  const handleDownloadZip = () => {
    downloadPlanillaZip(
      `Acta_Obtencion_${caso?.numeroCaso || 'EXP'}`,
      'Acta de Obtención por Consignación — SHA256.US'
    );
  };
  ...
  return (
    <div className="planilla-container">
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <span style={{ color: '#FECF06', fontWeight: 700, fontSize: '13px' }}>
              Acta de Obtención por Consignación (MUCC-2017)
            </span>
          </Box>
      ...
      {tabIndex === 0 ? (
        ...
        <PlanillaPdfViewer
          pdfBlob={pdfBlob}
          title={`Acta de Obtención — Caso #${caso?.numeroCaso || 'N/A'}`}
      ...
      <PlanillaGatingDialog
        ...
        nombrePlanilla="Acta de Obtención por Consignación"
      />
    </div>
  );
};
