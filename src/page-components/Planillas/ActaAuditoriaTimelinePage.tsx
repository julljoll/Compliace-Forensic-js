...
const ActaAuditoriaTimelinePage = () => {
  ...
  const handleDownloadZip = () => {
    downloadPlanillaZip(
      `Acta_Auditoria_${caso?.numeroCaso || 'EXP'}`,
      'Acta de Auditoría e Inmutabilidad de Evidencia Digital — SHA256.US'
    );
  };

  return (
    <div className="planilla-container">
      {/* BARRA SUPERIOR CYBER-LEGAL BLUEPRINT Y SELECTOR DE CASO */}
      <Box className="no-print" sx={{ width: '100%', maxWidth: '216mm', mb: 2 }}>
        ...
        >
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <span style={{ color: '#FECF06', fontWeight: 700, fontSize: '13px' }}>
              Acta de Trazabilidad y Cadena de Custodia Digital (Hash Chain)
            </span>
          </Box>
        ...
        {/* CONTROLES DE EXPEDIENTE Y DESCARGAS */}
        <Box
          sx={{
            display: 'flex',
            justify: 'space-between',
      ...
      {tabIndex === 0 ? (
        ...
        <PlanillaPdfViewer
          pdfBlob={pdfBlob}
          title={`Acta de Auditoría — Caso #${c.numeroCaso || 'N/A'}`}
