  ...
  return (
    <div className="space-y-8 apple-fade-in pb-20">
      ...
      <div className="flex justify-between items-end gap-4 border-b border-[var(--co-separator)] pb-4">
        ...
        {!showCrearCaso && (
          <Button variant="primary" size="sm" onClick={() => router.push('/casos?nuevo=true')} className="mb-1">
                  ...
                  return (
                    <tr key={caso.id} className={`hover:bg-[var(--co-surface-2)]/50 transition-colors h-14 ${isSelected ? 'bg-[var(--co-accent)]/[0.04]' : ''}`}>
                      ...
                      <td className="px-4 py-3">
                        <Link href={`/casos/${caso.id}`} className="font-mono text-[11px] font-semibold text-[var(--co-accent)] hover:underline">
                          {caso.numeroCaso}
                        </Link>
                      </td>
                      ...
                      <td className="px-4 py-3 text-[13.5px] font-semibold text-[var(--apple-text)] truncate max-w-[200px]" title={caso.titulo}>
                        <Link href={`/casos/${caso.id}`} className="hover:underline">{caso.titulo}</Link>
                      </td>
                      <td className="px-4 py-3">
                      ...
                      <td className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-1">
                          <Link href={`/casos/${caso.id}`} className="p-1.5 rounded-lg bg-[var(--co-surface-2)] text-[var(--co-gray-1)] hover:bg-[var(--co-accent)] hover:text-white transition-all">
              ...
              return (
                <Card key={caso.id} className={`flex flex-col p-5 h-full relative transition-all ${isSelected ? 'border-[var(--co-accent)] ring-1 ring-[var(--co-accent)] bg-[var(--co-accent)]/[0.02]' : ''}`} hoverable={true}>
                  ...
                  <div className="mt-auto pt-3 border-t border-[var(--co-separator)]">
                    ...
                    <div className="flex gap-2">
                      <Link href={`/casos/${caso.id}`} className="flex-1 flex justify-center items-center py-2 rounded-lg bg-[var(--co-accent)] text-white font-semibold text-[13px] transition-all hover:brightness-105 active:scale-95 shadow-sm">
                        Ver Detalles
                      </Link>
